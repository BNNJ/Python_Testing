...
def test_valid_purchase(mocker):
    ...
    competition = get_item(
        server.competitions, lambda x: x["name"] == "simple_competition"
    )
...
def test_not_enough_points(mocker):
    ...
    server.app.testing
    with server.app.test_client() as c:
        data = {"club": club["name"], "competition": competition["name"], "places": 10}
        r = c.post("/purchasePlaces", data=data)
...
def test_more_than_12_places(mocker):
    ...
    server.app.testing
    with server.app.test_client() as c:
        data = {"club": club["name"], "competition": competition["name"], "places": 15}
        r = c.post("/purchasePlaces", data=data)
        flashed_messages = get_flashed_messages()
